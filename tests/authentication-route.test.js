...
database.query = async (text, params) => {
    ...
    if (text.includes("FROM access_points")) {
        return {
            rowCount: 1,
            rows: [{ id: "ap-1", name: params[0] || "entry_gate_a", location: "Residence A", security_level: "high", status: "active" }]
        };
    }
...
database.getClient = async () => ({
    query: async (text, params) => {
        ...
        if (text.includes("INSERT INTO risk_assessments")) {
            return {
                rowCount: 1,
                rows: [{ id: "risk-1", risk_score: params[1], risk_level: params[2], factors: JSON.parse(params[3]), evaluated_at: new Date().toISOString() }]
            };
        }
        ...
        if (text.includes("INSERT INTO fusion_results")) {
            ...
            return {
                rowCount: 1,
                rows: [{ id: "fusion-1", participating_modalities: params[1], fused_score: params[2], algorithm_version: params[3], evidence: JSON.parse(params[4]) }]
            };
        }
        ...
        if (text.includes("INSERT INTO decisions")) {
            ...
            return {
                rowCount: 1,
                rows: [{ id: "decision-1", outcome: params[3], rationale: JSON.parse(params[4]), created_at: new Date().toISOString() }]
            };
        }
        ...
        if (text.includes("INSERT INTO access_controller_events")) {
            return {
                rowCount: 1,
                rows: [{ id: "controller-1", decision_id: params[0], target_resource: params[1], controller_response: params[2], delivered_at: new Date().toISOString(), response_payload: JSON.parse(params[3]) }]
            };
        }
...
test("authentication route uses biometric engine fusion when biometric samples are supplied", async () => {
    insertState = {};
        ...
        return {
            ...
            json: async () => ({
                ...
                fusion: {
                    ...
                    decision: "accept",
                    evidence: [{ modality: "face", quality_score: 91, normalized_score: 0.92, weight: 1 }],
    ...
    try {
        const response = await fetch(`${baseUrl}/api/authentication/attempts`, {
            ...
            body: JSON.stringify({
                ...
                modalityScores: { face: 20, gait: 10 },
        ...
        assert.equal(insertState.fusedScore, 92);
